...
def airfoil_task(angle, n_nodes, n_levels, speed, time):
    ...
    if not glob(mesh_xml_file):
        runme = subprocess.check_output([
            f'{GMSH_DIR}/runme.sh', 
            f'{angle} {angle} 1 {n_nodes} {n_levels}'])
        convert = subprocess.check_output([
            f'{GMSH_DIR}/dolfin-convert', 
            f'{GMSH_DIR}/msh/r{n_levels}a{angle}n{n_nodes}.msh', mesh_xml_file])
    airfoil = subprocess.check_output([
        f'{AIRFOIL_DIR}/airfoil',
        f'0 0.0001 {speed} {time}', mesh_xml_file])
